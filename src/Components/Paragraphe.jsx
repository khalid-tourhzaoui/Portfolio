...
export default function Paragraphe({ partie }) {
  if (partie == 0)
    return (
      <motion.div
        className="text-start self-start w-[75%] font-customFont text-1xl text-gray-100 backdrop-blur-[3px]"
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
    ...
    return (
      <motion.div
        className="text-start w-5/6 font-customFont text-sm md:text-base text-gray-100 backdrop-blur-[3px]"
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        transition={{ duration: 1, ease: 'easeInOut' }}
      >
        I&apos;m a recent graduate in Computer Engineering and Digital Governance from the Higher School of Technology in Kénitra 
        <strong className="text-orange-600"> (EST)</strong>, with a strong background in Web Development. I also hold a specialized technician diploma in Digital 
        Development (Web Full Stack) from <strong className="text-orange-600">OFPPT</strong>. Passionate about both software engineering and web technologies,
